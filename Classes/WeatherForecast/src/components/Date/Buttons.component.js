      ...
      {groupList.map((item, i) => {
        if (item.detail.length !== 0) {
          ...
          return (
            <div key={i}>
              <Button
                style={{ borderRadius: 6, margin: "5px 0px" }}
                onClick={() => {
                ...
                color="primary"
              >
                <p className="textInsideButton">
                  {" "}
                  {`${item.day}, `}
                  <span className="currentDate">
                    {new Date(item.detail[0].dt * 1000).getDate()}
                  </span>
                </p>
              </Button>
              {item.show ? (
                <p className="timeline">
                  {item.detail.map((it, i) => {
                    return (
                      <Button
                        key={i}
                        onClick={() => {
                          console.log(
                            "item.detail for current from button",
                            it
                          );
                          console.log(
                            "type off setCurrentWeather",
                            typeof setCurrentWeather
                          );
